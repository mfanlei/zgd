...
class BulletClusterModel:
    ...
    def print_report(self):
        ...
        # ===== 物理根源验证（新增） =====
        # 点1: 次团自身核心区 (r = rc2) 的牛顿项和几何项
        r_sub = self.rc2
        M2_enc_at_rc = M_beta_fast(r_sub, self.M2, self.rc2, self.beta2)
        gN_sub_core = G * M2_enc_at_rc / r_sub**2
        gGeo_sub_core = np.sqrt(G * M2_enc_at_rc * C**2 / RP) / r_sub

        # 点2: 主团在次团中心位置 (bullet_x, 0) 的贡献
        # 次团中心点坐标: (self.bullet_x, 0.0)
        r_main_to_sub = np.sqrt((self.bullet_x - 0.0)**2 + (0.0 - 0.0)**2)  # 即 self.bullet_x
        M1_enc_at_sub = M_beta_fast(r_main_to_sub, self.M1, self.rc1, self.beta1)
        gN_main_at_sub = G * M1_enc_at_sub / r_main_to_sub**2
        gGeo_main_at_sub = np.sqrt(G * M1_enc_at_sub * C**2 / RP) / r_main_to_sub

        # 输出比较
        print(f"\n[物理根源验证]")
        print(f"  次团自身核心区 (r = rc2 = {r_sub/KPC:.0f} kpc):")
        print(f"    g_N = {gN_sub_core:.3e} m/s²")
        print(f"    g_geo = {gGeo_sub_core:.3e} m/s²")
        print(f"    g_geo / g_N = {gGeo_sub_core/gN_sub_core:.4f}")
        print(f"  主团在次团中心 (距离 {r_main_to_sub/MPC:.3f} Mpc) 的贡献:")
        print(f"    g_N = {gN_main_at_sub:.3e} m/s²")
        print(f"    g_geo = {gGeo_main_at_sub:.3e} m/s²")
        print(f"  主团贡献 / 次团自身核心区贡献 (g_N比): {gN_main_at_sub/gN_sub_core:.4f}")
        print(f"  结论: 主团气体引力在次团核心区可忽略，" 
              f"次团核心峰位由次团自身质量主导，几何修正项在核心区极弱。")
