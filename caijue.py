...
print(f"(b) λ=1.1248             :  χ²={chi2(t_bare*1.1248):9.2f}  /12={chi2(t_bare*1.1248)/12:6.2f}"
      f"   [附录F/表8: 187.1, 15.6]")
print(f"(c) λ最优 = {lam_star:.6f}      :  χ²min={c2min:9.2f}  /12={c2min/12:6.2f}"
      f"   [附录F: λ*=1.1248]")
...
try:
    ...
    print("   [论文表9: 11.51/9.59/8.22/7.19 | 12.94/10.79/9.24或9.25/8.09"
          " | 12.90/10.02/8.04/6.63 | 12.77/9.98/8.03/6.62]")
except ImportError:
    print("\n(e)(g) 未装 scipy，跳过双幂律与表9复现")

# ---------------------------------------------------------------
# (f) 残差结构诊断 → 附录F: 11/13点 |r/σ|≤2; 两个例外
# ---------------------------------------------------------------
...
n_ok = 0
for i in range(n):
    flag = "OK" if abs(d_all[i]) <= 2.0 else "<-- 例外"
    if abs(d_all[i]) <= 2.0:
        n_ok += 1
...
print(f"  |r/σ|≤2 的点数: {n_ok}/13   [附录F: 11个（|r/σ|≤2）]")
print(f"  #12 与裸几何一致性: r/σ = {(obs[11]-t_bare[11])/sig[11]:+.3f}  [附录F: 0.07σ]")
print(f"  预期例外: z=0.934 D_H (≈+4.5σ); z=2.33 D_H (≈−10.7σ, 与裸几何0.07σ一致)")
